...
@RunWith(SpringRunner.class) @WebMvcTest(FrontController.class) public class FrontControllerTest {
    ...
    @Test public void getAgentHistoryTest() throws Exception {
        mvc.perform(get("/api/front/history/1").contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk()).andExpect(jsonPath("$[0].temperature").value(66.6))
