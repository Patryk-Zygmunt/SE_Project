...
public class ServerInfoRepositoryTest {
    ...
    private ServerInfoRepository serverInfoRepository;
     ...
     @Test
    public void   getServerInfosByAgent_AgentIdOrderByInfoTimeTest(){
   ...
   entityManager.persist(serverInfoOlder);     
    entityManager.flush();           
    List<ServerInfo> serverInfo = serverInfoRepository.getServerInfosByAgent_AgentIdOrderByInfoTime(
            (Long) entityManager.getId(agent));
    assertEquals(2,serverInfo.size());
    assertEquals(LocalDateTime.of(LocalDate.of(2005,11,12), LocalTime.of(12,11,12,0)),serverInfo.get(1).getInfoTime());
     }
